...
package body Double_Buffer is
   ...
   begin
      -- Creo que sólo se invoca 2 o 3 veces al principio y siempre se cumple la condición elsif not Buffer.Is_Frozen then
       PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!!!Expose");

      Old_Pixmap_cairo_context := Gdk.Cairo.Create(Get_Window(Buffer));


      if Buffer.Use_Triple_Buffer then
       PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!! usa triple buffer");

         Old_Triple_cairo_context := Gdk.Cairo.Create(Get_Window(Buffer));

         if Gdk.Event.Events_Pending then
                   PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!!!events pendientes");

            if Buffer.Is_Frozen then
                      PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!!!is frozen");

               Buffer.Should_Update_On_Screen := True;
            else
                      PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!!!NOT  IS FROZEN");
         ...
         PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!!!NOT  IS FROZEN Sin triple buffer");
          Gdk.Cairo.Set_Source_Pixbuf(Old_Pixmap_cairo_context, Old_Pixmap, 0.0, 0.0);

      else
         PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!!!update scrren a true");
         Buffer.Should_Update_On_Screen := True;
      end if;
   ...
   begin
      ...
      PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!!!Thaw");
      -- creo que nunca se cumple esta condición
      if Buffer.Should_Update_On_Screen then
         PKG_debug.Escribir("!!!!!!!!!!!!!!!!!!!!!!!!!!!update true");
